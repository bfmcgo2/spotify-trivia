...
export const addUsers = async(id, update, cb) => {
  let add = await gameRef.doc(id).collection('users').add(update);
  console.log(add);
  cb(add.id)
}

export const addAnswers = async(id, uid, update) => {
  await gameRef.doc(id).collection('users').doc(uid).collection('answers').add({update})
}
