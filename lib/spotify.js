...
const playlist = '5P1ejq91GHnncdZ2j3RzvK';
...
const A_PLAYLIST = `https://api.spotify.com/v1/playlists/${playlist}`
...
export const login = (dispatch, redirect) =>{
  ...
  window.spotifyCallback = async(payload) => {
    ...
    setUserTokens(tokens, dispatch, redirect);
  }
}


// SWR FETCH CONFLICT
const setUserTokens = async({ access_token }, dispatch, redirect) => {
  ...
  return signInWithSpotify(user_data, dispatch, redirect);
}
...
export const getAPlaylist = async (token) => {
  return getRequest(A_PLAYLIST, token)
};
